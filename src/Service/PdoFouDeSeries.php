...
class PdoFouDeSeries {
    ...
    public function __construct($server, $bdd, $user, $mdp) {
        PdoFouDeSeries::$monPdo = new PDO($server.';'.$bdd, $user, $mdp, array(
            PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8'));
    }

    public function getLesSeries() {
        $res = PdoFouDeSeries::$monPdo->prepare('SELECT * FROM serie');
        $res->execute();
        $lesSeries = $res->fetchAll();
        return $lesSeries;
    }
    public function getNbSeries() {
        $res = PdoFouDeSeries::$monPdo->prepare('SELECT count(*) FROM serie');
        $res->execute();
        $nbSeries = $res->fetch();
        return $nbSeries[0];
    }
    ...
    {
        $res = PdoFouDeSeries::$monPdo->prepare('SELECT * FROM serie WHERE id = :id');
        $res->execute(['id' => $id]);
        $uneSerie = $res->fetch();
        return $uneSerie;
    }
    public function setLaSerie($laserie)
    {
        ...
        $res = PdoFouDeSeries::$monPdo->prepare($req);
        ...
        $res1 = PdoFouDeSeries::$monPdo->prepare($req1);
        $res1->execute();
        $laLigne = $res1->fetch();
        return $laLigne;
    }
